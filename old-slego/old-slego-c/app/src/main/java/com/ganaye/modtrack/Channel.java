...
public class Channel {
    ...
    private void calculateAmplitude() {
        ...
        if (instrument.volumeEnvelope.enabled)
            envVol = instrument.volumeEnvelope.calculateAmplitude(volEnvTick);
        ...
        int envPan = 32;
        if (instrument.panningEnvelope.enabled)
            envPan = instrument.panningEnvelope.calculateAmplitude(panEnvTick);
    ...
    private void trigger() {
        if (noteIns > 0 && noteIns <= module.numInstruments) {
            ...
            com.ganaye.modtrack.Sample sam = instrument.samples[instrument.keyToSample[noteKey < 97 ? noteKey : 0]];
